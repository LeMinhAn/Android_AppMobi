...
//        image.setImageBitmap(imageCoverBitmap);
    ...
    private View.OnClickListener onFabShareButtonListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            if (future == null) {
                //prepare the call
                future = Ion.with(RingToneDetailActivity.this)
        ...
        public void onClick(View v) {
            if (future == null) {
                if (!Utils.isExternalStorageWritable()) {
                    Toast.makeText(RingToneDetailActivity.this, R.string.error_no_storage, Toast.LENGTH_SHORT).show();
                    return;
                }
            ...
            future.withResponse().setCallback(new FutureCallback<Response<InputStream>>() {
                ...
                public void onCompleted(Exception e, Response<InputStream> result) {
                    ...
                    if (e == null && result != null && result.getResult() != null) {
                        try {
                            WallpaperManager.getInstance(RingToneDetailActivity.this).setStream(result.getResult());
            ...
            future.withResponse().setCallback(new FutureCallback<Response<InputStream>>() {
                ...
                public void onCompleted(Exception e, Response<InputStream> result) {
                    ...
                    if (e == null && result != null && result.getResult() != null) {
                        try {
                            //create a temporary directory within the cache folder
                            File dir = new File(RingToneDetailActivity.this.getCacheDir() + "/images");
                            if (!dir.exists()) {
                                dir.mkdirs();
                            }
                            ...
                            Uri contentUri = FileProvider.getUriForFile(RingToneDetailActivity.this, "com.mikepenz.fileprovider", file);

                            if (set) {
                                //get crop intent
                                Intent intent = WallpaperManager.getInstance(RingToneDetailActivity.this).getCropAndSetWallpaperIntent(contentUri);
                                //start activity for result so we can animate if we finish
                                RingToneDetailActivity.this.startActivityForResult(intent, ACTIVITY_CROP);
                            } else {
                                //share :D
                                ...
                                //start activity for result so we can animate if we finish
                                RingToneDetailActivity.this.startActivityForResult(Intent.createChooser(shareIntent, "Share Via"), ACTIVITY_SHARE);
                            }
    ...
    private void animateActivityStart() {
        ...
        showTitleAnimator.setListener(new CustomAnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                super.onAnimationEnd(animation);
                mTitlesContainer.startAnimation(AnimationUtils.loadAnimation(RingToneDetailActivity.this, R.anim.alpha_on));
                ...
                mFabShareButton.animate()
                        .translationX((-1) * Utils.pxFromDp(RingToneDetailActivity.this, 58))
                ...
                mFabDownloadButton.animate()
                        .translationX((-1) * Utils.pxFromDp(RingToneDetailActivity.this, 108))
                        .setStartDelay(ANIMATION_DURATION_MEDIUM)
                        .setDuration(ANIMATION_DURATION_MEDIUM)
                        .start();
            }
    ...
    private void animateReset(boolean error) {
        ...
        mFabShareButton.animate().translationX((-1) * Utils.pxFromDp(RingToneDetailActivity.this, 58)).setDuration(ANIMATION_DURATION_MEDIUM).start();
        mFabDownloadButton.animate().translationX((-1) * Utils.pxFromDp(RingToneDetailActivity.this, 108)).setDuration(ANIMATION_DURATION_MEDIUM).start();
    }
